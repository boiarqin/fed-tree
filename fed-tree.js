var FED_Chicago_data = [
    ...
    'cm' : [{'name' : 'Scott Lutzow', 'dates' :['Sept 2013' ,'present']}]},        
    {'name' : 'Caroline Bowden',
    'titles' : [{'title': 'srassociate', 'dates' : ['Jun 2008', 'Jul 2013']},{'title': 'mgr', 'dates' : ['Jul 2013', 'present']}],
    'cm' : [{'name' : 'Paul Karlik', 'dates' :['Oct 2010' ,'Jul 2014']},{'name' : 'Rob O\'Brien', 'dates' :['Jul 2014' ,'present']}]},
    {'name' : 'Mike Brault',
    'titles' : [{'title': 'mgr', 'dates' : ['Apr 2011', 'present']}],
    'cm' : [{'name' : 'Paul Karlik', 'dates' :['Apr 2011' ,'Jul 2014']}]},
    {'name' : 'Paul Karlik',
    'titles' : [{'title': 'srmgr', 'dates' : ['Oct 2010', 'Jul 2014']}],
    'cm' : [{'name' : 'Scott Lutzow', 'dates' :['Sept 2013' ,'Jul 2014']}]},
    {'name' : 'Jeremy Rawls',
...
var isCurrentPeriod = function(start, end, currentDate){
    var startDate = parseDate(start);
    var endDate = parseDate(end);
    return (startDate < currentDate && currentDate <= endDate);
}

var filterEmployeesByDate = function(data, date){
...
var createTreeFromData = function(data, root){
    var children = []
    for (var i=0; i < data.length; i++){
        person = data[i];
...
var createFEDTree = function(date){
    ...
    var employeeData = filterEmployeesByDate(FED_Chicago_data, date);
        //console.log(pplData);
    var root = {'name': null, 'title': null};
    var treeJson = createTreeFromData(employeeData, root);
    treeJson['name']='';
        //console.log(treeJson);
    createD3Tree(treeJson);
};
...
$(function() {
    ...
    var date = new Date(2012, $(".slider").slider("value"));
    $(".date").text(date.toLocaleDateString("en-US", {'month':'long', 'year':'numeric'}));
    createFEDTree(date.toString());
  });
